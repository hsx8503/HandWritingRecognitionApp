...
class PyQtMainEntry(QMainWindow, Ui_MainWindow):
    ...
    def readImage(self):#从文件对话框中选择图像，并且在标签控件中显示该图像
    ...
    @QtCore.pyqtSlot()
    def queryFrame(self):#从摄像头捕获当前帧，并在标签控件中显示该帧图像
    ...
    def predict_capture(self):#对捕获的图像进行预测，并在界面上显示预测结果
        ...
        img_bw = preprocess_and_extract_text1(self.captured)
        # convert to gray
        print("predict_capture节点2")

        result = predict(img_bw)
        print("predict_capture节点3")

        self.result_capture.setText(str(result))
    def predict_folder(self):#图片上传部分的图片处理
        if not hasattr(self, "captured"):
            return
        #self.cpatured = cv.cvtColor(self.captured, cv.COLOR_RGB2GRAY)
        ...
        # convert to gray
        print("predict_folder节点2")
        img_bw = preprocess_and_extract_text1(self.captured)
        result = predict(img_bw)
        print("predict_folder节点3")
